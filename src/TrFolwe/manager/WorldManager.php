...
final class WorldManager {
     ...
     * @throws \JsonException
     */
    public static function addAreaPos(string $areaName, array $pos) :void {
        extract($pos);
     ...
     * @return string|null
     */
    public static function inAreaPos(int $x, int $z, string $world) :?string {
        $yamlDatabase = WGuardian::getInstance()->getYamlDatabase();
        foreach($yamlDatabase->get("areaPos") as $areaName => $areaData) {
            $posExplode = array_map(fn($c) => $c * 1, explode(":", $areaData["pos"]));
            if(in_array($x, range($posExplode[0], $posExplode[1])) && in_array($z, range($posExplode[2], $posExplode[3])) && $world === $areaData["world"]) return $areaName;
        }
        return null;
    }
